    ...
    export default function Pasien({ pasiens }) { 
        const [refresh, setRefresh] = useState(false); // Set refresh to false initially
        const [localPasiens, setLocalPasiens] = useState(pasiens); // State for storing patients data locally
        const { auth } = usePage().props;
        console.log(pasiens); // Cek apakah pasien terdefinisi


        useEffect(() => {
            if (refresh) {
                // Fetch the updated patients data
                axios.get(route('pasiens.index'))
                    .then(response => {
                        setLocalPasiens(response.data.pasiens); // Update local state
